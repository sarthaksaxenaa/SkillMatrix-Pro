...
import {
  Upload, FileText, Zap, BookOpen, Youtube, Trophy, ChevronRight, Search, Target, LogOut,
  Mic, Briefcase, FileSearch, History, Trash2, Clock, CheckCircle2, Flame, ExternalLink, Briefcase as JobIcon
} from 'lucide-react';
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex justify-between items-center mb-10">
            ...
            <div className="flex items-center gap-4">
              <div className="hidden sm:flex items-center gap-2 bg-orange-50 text-orange-600 px-4 py-2 rounded-full border border-orange-100 font-bold shadow-sm">
                <Flame size={18} className="fill-orange-500" /> {streak} Day Streak
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                 <div className="glass-card p-8 bg-indigo-50/30 border-indigo-100">
                    ...
                    <div className="flex gap-3">
                       <button onClick={() => openJobSearch('linkedin')} className="flex-1 bg-white border border-slate-200 py-3 rounded-xl font-bold text-slate-600 hover:text-blue-700 hover:border-blue-200 transition-colors flex items-center justify-center gap-2 shadow-sm"><Search size={16}/> LinkedIn</button>
                       <button onClick={() => openJobSearch('indeed')} className="flex-1 bg-white border border-slate-200 py-3 rounded-xl font-bold text-slate-600 hover:text-blue-600 hover:border-blue-200 transition-colors flex items-center justify-center gap-2 shadow-sm"><Search size={16}/> Indeed</button>
                       <button onClick={() => openJobSearch('glassdoor')} className="flex-1 bg-white border border-slate-200 py-3 rounded-xl font-bold text-slate-600 hover:text-green-600 hover:border-green-200 transition-colors flex items-center justify-center gap-2 shadow-sm"><Search size={16}/> Glassdoor</button>
                    </div>
                 </div>

                 <div className="glass-card p-8 bg-purple-50/30 border-purple-100 flex items-center justify-between">
                    <div>
                      <h3 className="font-bold text-purple-900 mb-1 flex items-center gap-2"><Zap size={20}/> Resume Fixer AI</h3>
                      <p className="text-purple-700 text-sm opacity-80">Generate better bullet points instantly.</p>
                    </div>
                    <button onClick={handleFixResume} className="bg-purple-600 text-white px-6 py-3 rounded-xl font-bold hover:bg-purple-700 transition-colors shadow-lg shadow-purple-200 flex items-center gap-2">
                       Fix My Resume <Zap size={16} className="fill-white"/>
                    </button>
                 </div>
              </div>
          ...
          {showFixModal && (
            <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
              <div className="bg-white rounded-3xl p-8 max-w-lg w-full shadow-2xl relative">
                 <button onClick={() => setShowFixModal(false)} className="absolute top-4 right-4 text-slate-400 hover:text-slate-600">
                 ...
                 <h3 className="text-2xl font-bold text-center text-slate-800 mb-2">AI Resume Upgrade</h3>
                 <p className="text-center text-slate-500 mb-8">Copy these "Power Bullets" to your resume.</p>
                 
                 {isFixing ? (
                    <div className="flex flex-col items-center py-8">
                       <div className="w-10 h-10 border-4 border-purple-600 border-t-transparent rounded-full animate-spin mb-4"></div>
                       <p className="text-purple-600 font-bold animate-pulse">Polishing your achievements...</p>
                    </div>
                 ) : (
                    <div className="space-y-4">
                       {fixedPoints.map((point, i) => (
                          <div key={i} className="bg-slate-50 p-4 rounded-xl border border-slate-200 text-slate-700 font-medium flex gap-3">
                             <div className="mt-1 text-green-500"><CheckCircle2 size={18} /></div>
                             {point}
                          </div>
                       ))}
                       <button onClick={() => setShowFixModal(false)} className="w-full bg-slate-900 text-white py-4 rounded-xl font-bold mt-4 hover:bg-slate-800">
                          Done
                       </button>
                    </div>
                 )}
